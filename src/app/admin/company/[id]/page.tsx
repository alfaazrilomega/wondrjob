...
export default function CompanyProfilePage() {
  ...
  const handleStatsUpdate = async (e: React.FormEvent) => {
    ...
    const res = await fetch(`/api/company/stats/${company.id}`, {
      ...
      body: JSON.stringify({
        year: statsYear,
        month: statsMonth,
        successRate: Number(statsRate || 0) / 100,
      }),
    });
    ...
    if (res.ok) {
      ...
      setCompany((prev) => {
        if (!prev) return null;
        const existingStatIndex = prev.monthlyStats.findIndex(
          (s) =>
            s.year === updatedStat.data.year &&
            s.month === updatedStat.data.month,
        );
        const newStats = [...prev.monthlyStats];
        if (existingStatIndex > -1)
          newStats[existingStatIndex] = updatedStat.data;
  ...
  const latestSuccessRate = useMemo(() => {
    if (!company || !company.monthlyStats || company.monthlyStats.length === 0)
      return 0;
    const sortedStats = [...company.monthlyStats].sort(
  ...
  return (
    <>
      ...
      <div className="p-8 text-white" style={{ background: "#101018" }}>
        <div className="glass-pane flex items-center justify-between p-6 mb-8">
          ...
          <button
            ...
            className={`px-6 py-2 rounded-lg font-semibold transition-all duration-300 ${isFollowing ? "border-2 border-[#9F54FF] text-[#9F54FF]" : "bg-[#9F54FF] text-white shadow-[0_0_15px_rgba(159,84,255,0.5)] hover:bg-purple-600"}`}
          >
              ...
              <div className="flex flex-col">
                {company.jobs && company.jobs.length > 0 ? (
                  company.jobs.map((job, index) => (
                    <div
                      key={job.id}
                      className={`flex items-center justify-between py-4 ${index < company.jobs.length - 1 ? "border-b border-[rgba(159,84,255,0.2)]" : ""}`}
                    >
                      <div>
                      ...
                      <Link href={`/admin/job-postings/${job.id}`} passHref>
                        <button className="px-5 py-2 rounded-lg border-2 border-[#9F54FF] text-[#9F54FF] font-semibold transition-all duration-300 hover:bg-[#9F54FF] hover:text-white">
                          View Details
                        </button>
                      </Link>
                    </div>
              ...
              <div className="flex justify-between items-center mb-2">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="px-4 py-1 rounded-lg border-2 border-[#9F54FF] text-[#9F54FF] font-semibold text-sm transition-all duration-300 hover:bg-[#9F54FF] hover:text-white"
                  >
                    Edit
                  </button>
                  <button
                    onClick={() => setIsDeleteModalOpen(true)}
                    className="px-4 py-1 rounded-lg border-2 border-red-500 text-red-500 font-semibold text-sm transition-all duration-300 hover:bg-red-500 hover:text-white"
                  >
                    Delete
                  </button>
                </div>
        ...
        <form onSubmit={handleStatsUpdate} className="flex flex-col gap-4">
          <div className="flex gap-4">
            <input
              ...
              onChange={(e) => {
                ...
                setStatsYear(isNaN(v) ? "" : v);
              }}
              className="w-full bg-transparent border border-white/20 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-[#9F54FF]"
            />
            <input
              type="number"
              placeholder="Month"
          ...
          <input
            ...
            onChange={(e) => {
              ...
              setStatsRate(isNaN(v) ? "" : v);
            }}
            className="w-full bg-transparent border border-white/20 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-[#9F54FF]"
          />
          <button
            type="submit"
            className="w-full px-6 py-2 rounded-lg bg-[#9F54FF] text-white font-semibold shadow-[0_0_15px_rgba(159,84,255,0.5)] transition-all duration-300 hover:bg-purple-600"
          >
            Save Changes
          </button>
        </form>
      ...
      <Modal
        title="Delete Monthly Stat"
        isOpen={isDeleteModalOpen}
        onClose={() => setIsDeleteModalOpen(false)}
      >
        <form onSubmit={handleStatsDelete} className="flex flex-col gap-4">
          <p className="text-sm text-white/70">
            Select the month and year of the stat you want to delete.
          </p>
          <div className="flex gap-4">
            <input
              type="number"
              ...
              onChange={(e) => {
                const v = parseInt(e.target.value);
                setDeleteYear(isNaN(v) ? "" : v);
              }}
              className="w-full bg-transparent border border-white/20 rounded-lg px-4 py-2 text-white focus:outline-none focus:ring-2 focus:ring-red-500"
            />
            <input
              type="number"
              placeholder="Month"
          ...
          <button
            type="submit"
            className="w-full px-6 py-2 rounded-lg bg-red-500 text-white font-semibold shadow-[0_0_15px_rgba(255,0,0,0.5)] transition-all duration-300 hover:bg-red-600"
          >
            Delete Stat
          </button>
        </form>
