...
export async function PUT(req: Request, context: { params: { id: string } }) {
  try {
    ...
    if (month === undefined || year === undefined || successRate === undefined) {
        return NextResponse.json(
            { success: false, error: "Missing required fields: month, year, successRate" },
            { status: 400 }
        );
    }
...
export async function DELETE(req: Request, context: { params: { id: string } }) {
  try {
    ...
    if (error.code === 'P2025') {
        return NextResponse.json(
            { success: false, error: "Stat entry not found for the given month and year." },
            { status: 404 }
        );
    }
    const errorMessage =
